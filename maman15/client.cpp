...
namespace
{
  ...
  constexpr uint32_t UNSIGNED(uint32_t n)
  {
    return n & 0xffffffff;
  }

  uint32_t memcrc(char *b, size_t n)
  {
    uint32_t s = 0;
    uint32_t tabidx;
    ...
    {
      tabidx = (s >> 24) ^ static_cast<unsigned char>(b[i]);
  ...
  uint32_t calc_crc(const std::string &fname)
  {
    if (std::filesystem::exists(fname))
    {
      std::ifstream f1(fname, std::ios::binary);

      uintmax_t file_size = std::filesystem::file_size(fname);
      if (file_size > static_cast<uintmax_t>(std::numeric_limits<uint32_t>::max()))
      {
        log("File is too large: ", fname);
        return 1;
      }
      uint32_t size = static_cast<uint32_t>(file_size);
      std::vector<char> buffer(size);
      f1.read(buffer.data(), size);
      return memcrc(buffer.data(), size);
    }
    else
    {
      log("Cannot open input file ", fname);
      return 1;
    }
  }
} // anonymous namespace
...
namespace maman15
{
  ...
  public:
    ...
    const bool send_file(const std::filesystem::path &file_path)
    {
      ...
      std::future<uint32_t> crc_future = std::async(calc_crc, file_path.string());
