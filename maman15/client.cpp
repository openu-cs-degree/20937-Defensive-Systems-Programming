...
#define UNSIGNED(n) (n & 0xffffffff) // TODO: eww
...
unsigned long calc_crc(std::string fname)
{
  if (std::filesystem::exists(fname))
  {
    ...
    char *b = new char[size]; // TODO: where delete?
...
namespace maman15
{
  ...
  public:
    ...
    const bool send_file(const std::filesystem::path &file_path)
    {
      ...
      std::future<unsigned long> crc_future = std::async(calc_crc, file_path.string());
